...
Vagrant.configure("2") do |config|
  ...
  config.vm.define :client do |client|
    client.vm.hostname = 'frontend.local'
    client.vm.network "forwarded_port", guest: 443, host: 8080
  ...
  config.vm.define :backend do |client|
    client.vm.hostname = 'backend.local'
